...
按理来说OTG是肯定可用的，但我没仔细研究，开启OTG模式后默认的host是可用的，但配置id引脚后无法自动切换模式，也可能只是我配错了，主线的musb驱动好像没有应用层切换。

目前RNDIS的USB从机需要固定OTG为peripheral模式并关闭host控制器才可以正常开启。
